        ...
        <div class="row">
                    ...
                    <div class="form-group col-5">
                        ...
                        <select name="buildSelect" id="buildSelect" class="form-control">
                            ...
                            @foreach($plan->builds as $build)
                                <option value="{{ json_encode(['title' => $build->title, 'color' => $build->color, 'id' => $build->id]) }}">{{ $build->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-5">
            ...
            <div class="col-12">
                <div class="row">
                    @foreach($plan->builds as $build)
                        <div class="col-auto">
                            <div class="">{{ $build->title }} <div class="border" style="width: 20px; height: 20px; display:inline-block; background: {{ $build->color }}"></div></div>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-12 px-0 mb-5 ">
                <div id="image-preview" class="bg-dark">
                    <img src="{{ asset('storage/'.$plan->image) }}" id="image-preview-img" class="w-100" alt="">
    ...
    <script>
        $(document).ready(e => {
            $.ajax({
                ...
                success: data => {
                    setTimeout(function () {
                        let width = $('#image-preview-img').width();
                        let height = $('#image-preview-img').height();
                        ...
                        for (let item of data) {
                            let dot = '<div class="dot" data-id=' + item.id + ' style=top:' + parseInt(ratio * item.top) + 'px;left:' + parseInt(ratio * item.left) + 'px;width:' + parseInt(ratio * 15) + 'px;height:' + parseInt(ratio * 15) + 'px;background:' + item.build.color + ';></div>';
                            $('.backzone').append(() => {
                                return registerDraggable($(dot));
                            });
                        }
